...
import { isDueTomorrow } from "@/lib/db/queries";
...
export default async function TomorrowPage() {
  ...
  const filtered = (items ?? []).filter((i) =>
    isDueTomorrow(i as unknown as InboxItem)
  );

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold text-[var(--foreground)]">Tomorrow</h1>
        <p className="mt-1 text-sm text-[var(--muted)]">Due tomorrow</p>
      </div>
      {!filtered.length ? (
        <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-8 text-center">
          <p className="text-[var(--muted)]">Nothing due tomorrow.</p>
        </div>
      ) : (
